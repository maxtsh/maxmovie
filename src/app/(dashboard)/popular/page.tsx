import { getMovieGenresData, getPopularData } from "@/service";
...
const PopularPage: React.FC = async () => {
  const movies = await getPopularData({ page: initalPage });
  ...
  const fetchMore = async (page: number) => {
    "use server";
    return getPopularData({ page });
  };

  return (
    <section className="flex flex-1 flex-col gap-2">
      <div className="p-2">
        <h2 className="font-extrabold">Most Popular</h2>
      </div>
